    ...
    with open(filename) as f:
        ...
        while (
            ...
            if line.startswith("TYPE"):
                assert (
                    "TSP" in line.split()
                    or "ATSP" in line.split()
                    or "SOP" in line.split()
                )
...
def read_multi_demand(n, f):
    line = f.readline()
    while line:
        if line.startswith("DEMAND_DIMENSION"):
            dimension = int(line.split()[-1])
        if line.startswith("DEMAND_SECTION"):
            break
        line = f.readline()

    items = list(range(1, dimension + 1))
    demand = {}
    ...
    for _ in range(n):
        ...
        for j in range(dimension):
            demand[i, j + 1] = int(line[j + 1])

    return dimension, items, demand


def read_mpdtsp(filename):
    with open(filename) as f:
        n = None
        edge_weight_type = None
        edge_weight_format = None
        ...
        f.seek(0)
        m, items, demand = read_multi_demand(n, f)

    return n, nodes, edges, capacity, m, items, demand, symmetric


def validate_mpdtsp(solution, cost, nodes, edges, capacity, items, demand):
    unvisited = set(nodes)
    load = {i: 0 for i in items}
    actual_cost = 0
    previous = None
    ...
    for i in solution:
        if previous is None:
            if i != nodes[0]:
                print(f"The tour does not start from depot {nodes[0]}, but from {i}")
                return False
        else:
            if (previous, i) not in edges:
                print(f"No such edge ({previous}, {i})")
        ...
        if i not in nodes:
            print(f"No such node {i}")
            return False

        if i not in unvisited:
            print(f"Node {i} is already visited")
            return False
        ...
        for j in items:
            if load[j] < 0:
                print(f"The weight {load[j]} for item {j} is negative")
                return False
        ...
        if load_sum > capacity:
            print(
                f"The sum of weights {load_sum} at node {i} exceeds the capacity {capacity}"
            )
            return False
    ...
    if len(unvisited) > 0:
        print(f"Nodes {unvisited} are unvisited")
        return False

    if i != nodes[-1]:
        print(f"The tour does not end at depot {nodes[-1]}, but at {i}")
